// subjectOptions.js

const subjectOptions = {
    //primary school
    beez: [
        'BengaliW', 'BengaliL', 'BengaliP', 'BengaliA',
        'EnglishW', 'EnglishL', 'EnglishP', 'EnglishA',
        'MathsW', 'MathsL', 'MathsP',
        'Physi'
    ],
    ankur: [
        'BengaliW', 'BengaliS', 'BengaliP', 'BengaliA',
        'EnglishW', 'EnglishS', 'EnglishP', 'EnglishA',
        'Maths', 'Work', 'Physi'
    ],
    kisholoy: [
        'BengaliW', 'BengaliS', 'BengaliP', 'BengaliA',
        'EnglishW', 'EnglishS', 'EnglishP', 'EnglishA',
        'Maths', 'Work', 'Physi'
    ],
    1: [
        'BengaliW', 'BengaliS', 'BengaliP', 'BengaliA',
        'EnglishW', 'EnglishS', 'EnglishP', 'EnglishA',
        'Maths', 'GKW', 'GKO', 'Draw', 'Work', 'Physi'
    ],
    2: [
        'BengaliW', 'BengaliS', 'BengaliP', 'BengaliA',
        'EnglishW', 'EnglishS', 'EnglishP', 'EnglishA',
        'Maths', 'HistoryW', 'HistoryO', 'GrographyW', 'GrographyO',
        'NitiW', 'NitiO', 'GKW', 'GKO', 'Draw', 'Work', 'Physi'
    ],
    3: [
        'Bengali1W', 'Bengali2O', 'Bengali2W', 'Bengali2O',
        'English1W', 'English1O', 'English2W', 'English2O',
        'Maths', 'HistoryW', 'HistoryO', 'GrographyW', 'GrographyO',
        'ScienceW', 'ScienceO', 'NitiW', 'NitiO', 'GKW', 'GKO',
        'ComputerW', 'ComputerO', 'Draw', 'Work', 'Physi'
    ],
    4: [
        'Bengali1W', 'Bengali2O', 'Bengali2W', 'Bengali2O',
        'English1W', 'English1O', 'English2W', 'English2O',
        'Maths', 'HistoryW', 'HistoryO', 'GrographyW', 'GrographyO',
        'ScienceW', 'ScienceO', 'NitiW', 'NitiO', 'GKW', 'GKO',
        'ComputerW', 'ComputerO', 'Draw', 'Work', 'Physi'
    ],

    //high school
    5: [
        'FirstLangW', 'FirstLangO', 'SecondLangW', 'SecondLangO',
        'ThirdLangW', 'ThirdLangO', 'Maths', 'ScienceW', 'ScienceO',
        'GrographyW', 'GrographyO', 'HistoryW', 'HistoryO', 'GKW',
        'GKO', 'ComputerW', 'ComputerO', 'Work', 'Physi'
    ],

    6: ['FirstLangW', 'FirstLangO', 'SecondLangW', 'SecondLangO',
        'ThirdLangW', 'ThirdLangO', 'Maths', 'ScienceW', 'ScienceO',
        'GrographyW', 'GrographyO', 'HistoryW', 'HistoryO', 'GKW',
        'GKO', 'ComputerW', 'ComputerO', 'Work', 'Physi'],

    7: ['FirstLangW', 'FirstLangO', 'SecondLangW', 'SecondLangO',
        'ThirdLangW', 'ThirdLangO', 'Maths', 'ScienceW', 'ScienceO',
        'GrographyW', 'GrographyO', 'HistoryW', 'HistoryO', 'GKW',
        'GKO', 'ComputerW', 'ComputerO', 'Work', 'Physi'],
        
    8: ['FirstLangW', 'FirstLangO', 'SecondLangW', 'SecondLangO',
        'ThirdLangW', 'ThirdLangO', 'Maths', 'ScienceW', 'ScienceO',
        'GrographyW', 'GrographyO', 'HistoryW', 'HistoryO', 'GKW',
        'GKO', 'ComputerW', 'ComputerO', 'Work', 'Physi']
};

export default subjectOptions;
